...
def Testing_Stage(network, root_dir, k=30):
    ...
    #     all_dist = np.linalg.norm((X_test[i]-X_train), ord=2, axis=1)
    ...
    start = time()
    part = 100
    rounds = int(len(X_test)/part)
    for i in range(rounds):
        neighbors_index = neigh.kneighbors(X_test[i*part: (i+1)*part], return_distance=False)
        labels_neigh = [[y_train[m] for m in neighbors_index[n]] for n in range(part)]
        for j in range(part):
            correct = np.sum(np.array(labels_neigh[j]) == y_test[i*part+j])
            precision = correct/k
            all_precision.append(precision)
        print ("[%d/%d]: %.4f%%, cost: %.4fs"%(i+1, len(X_test)/part, 100*np.mean(all_precision), time()-start))
...
if __name__ == '__main__':
    ...
    print ("The overall accuracy of this model is: %.5f%%"%(precision))
