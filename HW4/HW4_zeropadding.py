...
def ShortCutProjection(x):
    channels = x.size(1)
    #Downsample the input x to match the size of 16 x 16
    downsampled_x = nn.functional.avg_pool2d(input=x, kernel_size=1, stride=2)
    #Pad zeros for increased channels
    padded_x = nn.functional.pad(input=downsampled_x, pad=(0,0,0,0,0,channels), mode="constant", value=0)
    return padded_x
